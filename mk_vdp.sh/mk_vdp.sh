...
do
  ...
  do
    ...
    echo "<<<<"${VM}">>>>"
    echo "<<<local>>>"
    echo ${EXITCODE}" VDP seconds="${DURATIONSEC}"|deltapercentage="${CHANGEDPERCENTAGE}" vSphere Data Protection Backup, VM: "${VM}", Appliance: "`hostname`", last run: "${ENDDATE}" "${ENDTIME}" "${ENDTZ}" (status: "${STATUS}", transferred "${CHANGEDPERCENTAGE}"% of "${SIZE}${UNIT}")"${ERR}${WARN}
